...
onmessage = function (e) {
  ...
  if (min === max) {
    // Для алгоритма волн установим значения так, чтобы был виден хотя бы контур
    if (min < 128) {
      max = 255; // Создаем контраст
    } else {
      min = 0; // Создаем контраст
    }
  }
  ...
  if (config.Direction != 'Horizontal') {
    for (let sx = -waveAmplitude; sx <= config.width + waveAmplitude; sx += lineSpacing) {
      ...
      for (let y = 0; y < config.height; y++) {
        ...
        hysteresis += getNormalizedPixel(nx, y) > threshold ? 1 : -1;
  ...
  if (config.Direction != 'Vertical') {
    for (let sy = -waveAmplitude; sy <= config.height + waveAmplitude; sy += lineSpacing) {
      ...
      for (let x = 0; x < config.width; x++) {
        ...
        hysteresis += getNormalizedPixel(x, ny) > threshold ? 1 : -1;
        if (hysteresis <= -simplify) hysteresis = -simplify;
        if (hysteresis > simplify) hysteresis = simplify;
